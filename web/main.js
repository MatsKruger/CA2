...
$('#saveNewCompany').on('click', function() {
    ...
    var json = {
        ...
        phones: phones,
        address: {
            street: address.adgangsadresse.vejstykke.navn,
            additionalInfo: address.husnr + (address.etage ? ', ' + address.etage : '') + (address.dør ? '. ' + address.dør : ''),
            cityInfo: {
                zipCode: address.adgangsadresse.postnummer.nr,
                city: address.adgangsadresse.postnummer.navn
            }
        }
    };
});
...
$('#newCompanySearch').on('keypress', function(e) {
    if (e.keyCode === 13) {
        $.getJSON('http://cvrapi.dk/api?search=' + this.value + '&country=DK', function(data) {
            ...
            var maxEmployees = (data.employees ? Math.max.apply(Math, data.employees.split('-').map(function(item) {return parseInt(item);})) : null);
